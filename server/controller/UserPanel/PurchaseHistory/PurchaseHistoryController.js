...
const purchaseHistory = async (req, res) => {
  try {
    const { user } = req.body;
    const query = `select o.order_id ,o.id, p.photo as photo, p.id as product_id , o.quantity ,o.status, p.rating,TO_CHAR(o.created_at, 'YYYY-MM-DD') as date from sahil.order_history as o inner join sahil.products as p on o.product_id = p.id  where user_id = :id  `;
